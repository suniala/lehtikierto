...
import lehtikierto.client.Routes.ShareStatusLoc
    ...
    .render_P { proxy =>
      Panel(Panel.Props("Jakamani numerot"),
        ...
        proxy().renderReady(m => <.ul(m.toTagMod(
            Share => <.li(Routes.link(ShareStatusLoc(Share.id.id))(s"${Share.number.magazine.name} ${Share.number.year} ${Share.number.number}")))))
      )
    }
